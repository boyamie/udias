"""TEMP smoke test for the training path (rgb_only, 3 epochs).
Validates: autolabel-produced labels -> YOLO dataset export -> VRAM fit ->
training loop -> outputs. batch=8 probes whether AutoBatch's very
conservative estimate (3) is real; watch for CUDA OOM in the log.
Windows note: the __main__ guard is REQUIRED (spawn-mode DataLoader workers
